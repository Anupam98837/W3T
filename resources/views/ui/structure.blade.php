...
<head>
  ...
  <style>
    ...
    .w3-sidebar{
      ...
      border-right:1px solid var(--w3-rail-border); display:flex; flex-direction:column; z-index:1051;
    ...
    .w3-appbar-inner{width:100%; display:flex; align-items:center; gap:10px; padding:0 12px}
    @media (min-width: 992px){
      .w3-appbar-inner{max-width:1200px; margin-inline:auto; margin-left:var(--w3-rail-w)}
    }
    ...
    /* Content (max 1200px) */
    .w3-content{padding:16px; max-width:1200px; margin-inline:auto; margin-left:var(--w3-rail-w); transition:margin .28s ease}

    /* Overlay (starts after sidebar; clicking it closes on mobile) */
    .w3-overlay{
    ...
    /* Notification dropdown “note” card */
    .w3-note{
      background:#f6fbff; border:1px solid #e9f2ff; border-radius:8px; padding:.5rem .6rem;
    }
    ...
    /* Dark flips (only for layout bits) */
    ...
    /* Make hamburger bars light in dark mode */
    html.theme-dark .w3-bar{ background:#e8edf7; }

    /* Dark dropdown & note card */
    ...
    html.theme-dark .w3-note{ background:#0c203f; border-color:#153a6b; color:var(--text-color); }
  </style>
</head>
<body>
...
<aside id="sidebar" class="w3-sidebar" aria-label="Sidebar">
  ...
  <div class="w3-sidebar-scroll">
    ...
    <nav class="w3-menu" aria-label="Overview">
      <a href="/" class="w3-link"><i class="fa-solid fa-gauge"></i><span>Dashboard</span></a>
    </nav>
    ...
    <nav class="w3-menu" aria-label="Academics">
      <div class="w3-group">
        ...
        <div id="sm-courses" class="w3-submenu" role="group" aria-label="Courses submenu">
          <a href="/admin/courses" class="w3-link">All Courses</a>
      ...
      <div class="w3-group">
        ...
        <div id="sm-students" class="w3-submenu" role="group" aria-label="Students submenu">
          <a href="/admin/students" class="w3-link">Manage Students</a>
          <a href="/admin/batches" class="w3-link">Batches</a>
    ...
    <nav class="w3-menu" aria-label="Operations">
      ...
      <a href="/admin/settings" class="w3-link"><i class="fa-solid fa-gear"></i><span>Settings</span></a>
    </nav>
    ...
    <div class="ms-auto d-flex align-items-center gap-2">
      ...
      <div class="dropdown d-none d-lg-block">
        <a href="#" class="btn btn-primary rounded-pill d-flex align-items-center gap-2 px-3" id="userMenu" data-bs-toggle="dropdown" aria-expanded="false">
          <i class="fa-regular fa-user"></i> <span class="d-none d-xl-inline">Admin</span>
        </a>
...
<main class="w3-content">
  <section class="panel mb-3">
    <div class="d-flex align-items-center justify-content-between gap-2">
      <div>
        <h2 class="m-0" style="font-family:var(--font-head);font-weight:600;color:var(--ink)">@yield('title','Dashboard')</h2>
        <div class="text-muted">@yield('subtitle','Operate your campus smoothly.')</div>
      </div>
      <div>@yield('actions')</div>
    </div>
    @hasSection('breadcrumb')
      <nav aria-label="breadcrumb" class="mt-2"><ol class="breadcrumb m-0">@yield('breadcrumb')</ol></nav>
    @endif
  </section>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Submenus (don’t close sidebar on mobile when toggling)
  ...
  // Active link + auto-open parent submenu
  ...
  // On mobile, clicking a leaf link closes the sidebar; toggles do not.
  const isMobile = () => window.matchMedia('(max-width: 991px)').matches;
  document.querySelectorAll('.w3-menu .w3-link').forEach(link => {
    link.addEventListener('click', () => {
      if (link.classList.contains('w3-toggle')) return;
      if (isMobile()) closeSidebar();
    });
  });

  // Dummy logout actions
  document.getElementById('logoutBtn')?.addEventListener('click', e => { e.preventDefault(); window.location.href='/'; });
  document.getElementById('logoutBtnSidebar')?.addEventListener('click', e => { e.preventDefault(); window.location.href='/'; });
});
</script>
</body>
</html>
