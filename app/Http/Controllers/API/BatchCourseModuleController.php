...
class BatchCourseModuleController extends Controller
{
    ...
    /* =========================================================
     ...
     |  - Inserts ONLY columns that exist (schema-safe)
     |=========================================================*/
    ...
    private function logActivity(
        ...
        try {
            ...
            // common columns (best effort across your possible schemas)
            if (Schema::hasColumn('activity_logs', 'uuid')) $ins['uuid'] = (string) Str::uuid();

            if (Schema::hasColumn('activity_logs', 'module')) $ins['module'] = 'batch_course_module';
            if (Schema::hasColumn('activity_logs', 'action')) $ins['action'] = $action;
            if (Schema::hasColumn('activity_logs', 'message')) $ins['message'] = $message;

            // actor id columns
            foreach (['actor_id','user_id','created_by'] as $col) {
                if (Schema::hasColumn('activity_logs', $col)) { $ins[$col] = (int)$uid; break; }
            }
            // actor role columns
            foreach (['actor_role','role'] as $col) {
                if (Schema::hasColumn('activity_logs', $col)) { $ins[$col] = (string)$role; break; }
            }
    ...
    private function resolveCourseIdFromRequest(Request $r, bool $required = false): ?int
    {
        ...
        if (is_array($cmIds) && count($cmIds) > 0) {
            ...
            if (count($courseIds) === 1) return (int) $courseIds[0];
    ...
    public function index(Request $r)
    {
        ...
        $bcmOrder = $this->bcmOrderColumn();
        $cmOrder  = $this->cmOrderColumn();
        ...
        if ($courseId) $q->where('bcm.course_id', $courseId);

        if ($r->filled('status')) $q->where('bcm.status', $r->status);
     ...
     */
    private function unassignModules(Request $r, string $reqId, int $batchId, int $courseId, array $moduleIds): \Illuminate\Http\JsonResponse
    {
        $now = now();

        // grab old active rows (best-effort, for activity log)
        $oldRows = DB::table('batch_course_module')
            ->whereNull('deleted_at')
            ->where('batch_id', $batchId)
        ...
        try {
            ...
            // ✅ ACTIVITY LOG
            $this->logActivity($r, 'unassign_modules', 'Modules unassigned from batch', [
                'req_id' => $reqId,
                'batch_id' => $batchId,
                'course_id' => $courseId,
                'module_ids' => $moduleIds,
                'affected_rows' => (int)$affected,
            ], 'batch_course_module', null, $oldRows->toArray(), null);
    ...
    public function update(Request $r, string $idOrUuid)
    {
        ...
        if ($incomingOrder !== null && $incomingOrder !== '') {
            ...
            if (Schema::hasColumn('batch_course_module', 'display_order')) {
    ...
    public function toggleCompleted(Request $r, string $idOrUuid)
    {
        ...
        // ✅ ACTIVITY LOG
        $this->logActivity($r, 'toggle_completed', 'Batch module completion toggled', [
            'batch_course_module_id' => (int)$row->id,
            'new_is_completed' => (int)$newState,
        ], 'batch_course_module', (int)$row->id, $old, $updated);
    ...
    public function destroy(Request $r, string $idOrUuid)
    {
        ...
        $new = DB::table('batch_course_module')->where('id', (int)$row->id)->first();

        // ✅ ACTIVITY LOG
        $this->logActivity($r, 'delete_batch_module', 'Batch module deleted (soft)', [
            'batch_course_module_id' => (int)$row->id,
            'batch_id' => (int)($row->batch_id ?? 0),
            'course_id' => (int)($row->course_id ?? 0),
            'course_module_id' => (int)($row->course_module_id ?? 0),
        ], 'batch_course_module', (int)$row->id, $old, $new);
    ...
    public function upsertSettings(Request $r, string $batchUuid)
    {
        ...
        try {
            ...
            // ✅ ACTIVITY LOG
            $this->logActivity($r, 'upsert_settings', 'Batch module settings saved', [
                'mode' => $mode,
                'batch_uuid' => $batchUuid,
                'batch_id' => (int)$batch->id,
                'course_id' => (int)$courseId,
                'settings' => $settings,
            ], 'batch_course_module_settings', (int)($saved->id ?? 0), $old, $saved);
    ...
    public function unlockStatus(Request $r)
    {
        ...
        if ($mods->isEmpty()) {
            // ✅ ACTIVITY LOG
            $this->logActivity($r, 'unlock_status', 'Unlock status checked (no modules found)', [
                'batch_id' => $batchId,
                'course_id' => $courseId,
                'count' => 0,
            ], 'batch_course_module', null);
        ...
        // ✅ ACTIVITY LOG
        $this->logActivity($r, 'unlock_status', 'Unlock status checked', [
            'batch_id' => $batchId,
            'course_id' => $courseId,
            'count' => count($out),
        ], 'batch_course_module', null);
